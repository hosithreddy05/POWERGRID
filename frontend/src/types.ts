 ...
 * POWERGRID Project Intelligence (PowerPredict)
 */
...
export interface ProjectSnapshot {
  ...
  cumulative_expenditure: number; // in ₹ Cr
  elapsed_duration_months: number;
  progress_velocity: number; // % / month
  expenditure_velocity: number; // ₹ Cr / month
  risk_score: number;
...
export interface PowerGridProject {
  ...
  original_approved_cost: number; // in ₹ Cr
  cumulative_expenditure: number; // in ₹ Cr
...
export interface WhatIfComparison {
  ...
  baseline: {
    inputs: Partial<PowerGridProject>;
